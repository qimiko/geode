...
// gd imports this from a library, so maybe we should do this too?
void* gd::operatorNew(size_t size) {
    static auto fnPtr = reinterpret_cast<void*(*)(size_t)>(geode::base::get() + 0x2a1ace);
    return fnPtr(size);
}

void gd::operatorDelete(void* ptr) {
    static auto fnPtr = reinterpret_cast<void(*)(void*)>(geode::base::get() + 0x2a1ac2);
    return fnPtr(ptr);
}
