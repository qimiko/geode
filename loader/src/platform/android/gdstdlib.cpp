...
static struct {
    int a, b, c, d;
} empty_internal_string { 0, 0, 0, 0 };

gd::string::string() {
    m_data = reinterpret_cast<decltype(m_data)>(&empty_internal_string.d);
}

gd::string::string(char const* ok) {
    // reinterpret_cast<void (*)(string*, char const*)>(geode::base::get() + 0x489fc0)(this, ok);
}

gd::string::string(gd::string const&) {

}

gd::string& gd::string::operator=(gd::string const&) {
    return *this;
}

gd::string& gd::string::operator=(const char*) {
    return *this;
}
