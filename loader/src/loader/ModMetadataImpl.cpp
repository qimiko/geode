...
Result<ModMetadata> ModMetadata::Impl::createFromSchemaV010(ModJson const& rawJson) {
    ...
    if (auto internalBinary = root.has("internal-binary")) {
        if (internalBinary.isObject()) {
            auto key = PlatformID::toShortString(GEODE_PLATFORM_TARGET, true);
            if (internalBinary.has(GEODE_PLATFORM_SHORT_IDENTIFIER_NOARCH)) {
                internalBinary.needs(GEODE_PLATFORM_SHORT_IDENTIFIER_NOARCH).into(impl->m_internalBinary);
            }
        }
        else {
