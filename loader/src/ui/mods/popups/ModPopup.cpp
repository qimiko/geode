...
bool ModPopup::setup(ModSource&& src) {
    ...
    >> {
        ...
        { "discord", "gj_discordIcon_001.png"_spr, m_source.getMetadata().getLinks().getCommunityURL(), nullptr },
...
void ModPopup::onTab(CCObject* sender) {
    ...
    this->loadTab(static_cast<Tab>(nSender->getTag()));
}
...
void ModPopup::onEnable(CCObject*) {
    if (auto mod = m_source.asMod()) {
        ...
        if (!res) {
            FLAlertLayer::create("Error Toggling Mod", res.unwrapErr().c_str(), "OK")->show();
        }
    }
    else {
