...
void ConfirmUninstallPopup::onUninstall(CCObject*) {
    auto res = m_mod->uninstall(m_deleteDataToggle->m_toggled);

    if (res) {
        std::string msg = m_mod->getName() + " has been uninstalled!";
        FLAlertLayer::create(
            "Mod Uninstalled",
            msg.c_str(),
            "OK"
        ...
        std::string msg = "Error uninstalling " + m_mod->getName() + ": " + res.unwrapErr();
        FLAlertLayer::create(
            "Error Uninstalling",
            msg.c_str(),
            "OK"
